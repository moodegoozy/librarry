    ...
    for (const doc of ordersSnap.docs) {
        ...
        try {
            ...
            if (yakResult) {
                ...
                const items = yakResult.items || [];
                const trackingNumbers = items
                    .map((i) => i.trackingNumber || i.lastMileTracking)
                    .filter(Boolean);
...
const tap = __importStar(require("./tapClient"));
async function initTapKeys() {
    // أولاً: التحقق من Firestore
    const settingsDoc = await admin.firestore().doc("settings/tap").get();
    const settings = settingsDoc.data();
    if (settings === null || settings === void 0 ? void 0 : settings.secretKey) {
    ...
    throw new functions.https.HttpsError("failed-precondition", "مفاتيح Tap API غير مُعدة. يرجى إعدادها في الإعدادات.");
}
/**
 * تحويل رقم الجوال السعودي إلى صيغة تاب (country_code + number بدون صفر بادئ)
 */
function parseTapPhone(rawPhone) {
...
const TAP_WEBHOOK_URL = "https://us-central1-jabouri-digital-library.cloudfunctions.net/tapWebhook";
/**
 ...
 */
async function ensureTapOrderPaid(reference, chargeId, paymentMethodLabel) {
    var _a;
    const db = admin.firestore();
    ...
    if (existing.exists) {
        ...
        if (((_a = existing.data()) === null || _a === void 0 ? void 0 : _a.paymentStatus) !== "paid") {
...
exports.tapCreateCharge = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "يجب تسجيل الدخول لإتمام الدفع");
    }
    const { amount, currency, description, customer, order_reference_id, redirect_url, source_id, orderData, } = data;
    if (!amount || parseFloat(amount) <= 0) {
        throw new functions.https.HttpsError("invalid-argument", "المبلغ غير صحيح");
    }
    if (!order_reference_id) {
    ...
    if (!redirect_url) {
        throw new functions.https.HttpsError("invalid-argument", "رابط العودة مطلوب");
    }
    if (!(customer === null || customer === void 0 ? void 0 : customer.email) && !(customer === null || customer === void 0 ? void 0 : customer.phone)) {
        throw new functions.https.HttpsError("invalid-argument", "بيانات العميل مطلوبة (البريد الإلكتروني أو رقم الجوال)");
    }
    try {
        await initTapKeys();
        ...
        return {
            id: result.id,
            status: result.status,
            redirect_url: ((_a = result.transaction) === null || _a === void 0 ? void 0 : _a.url) || null,
        };
    }
    catch (error) {
...
exports.tapGetChargeStatus = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "يجب تسجيل الدخول");
    }
    ...
    try {
        ...
        const reference = order_reference_id || ((_a = result.reference) === null || _a === void 0 ? void 0 : _a.order);
        // إنشاء/تعليم الطلب كمدفوع في الخادم عند تأكيد الدفع فعلياً فقط
        if (result.status === "CAPTURED" && reference) {
            await ensureTapOrderPaid(reference, chargeId, (_b = result.source) === null || _b === void 0 ? void 0 : _b.payment_method);
        }
        return {
            id: result.id,
            status: result.status,
            ...
            payment_method: ((_c = result.source) === null || _c === void 0 ? void 0 : _c.payment_method) || null,
        };
    }
    catch (error) {
...
exports.tapWebhook = functions.https.onRequest(async (req, res) => {
    var _a, _b, _c;
    try {
        const body = req.body || {};
        const chargeId = body.id || ((_a = body.charge) === null || _a === void 0 ? void 0 : _a.id);
        if (!chargeId) {
        ...
        const reference = (_b = charge.reference) === null || _b === void 0 ? void 0 : _b.order;
        if (charge.status === "CAPTURED" && reference) {
            await ensureTapOrderPaid(reference, chargeId, (_c = charge.source) === null || _c === void 0 ? void 0 : _c.payment_method);
            console.log(`Tap webhook: order ${reference} marked paid (${chargeId})`);
        }
        else {
            console.log(`Tap webhook: charge ${chargeId} status=${charge.status} (no action)`);
        }
...
// ==================== Tap - حفظ إعدادات API ====================
exports.tapSaveSettings = functions.https.onCall(async (data, context) => {
    var _a;
    await verifyAdmin((_a = context.auth) !== null && _a !== void 0 ? _a : undefined);
    const { publicKey, secretKey } = data;
    if (!secretKey) {
...
// ==================== Tap - اختبار الاتصال ====================
exports.tapTestConnection = functions.https.onCall(async (data, context) => {
    var _a;
    await verifyAdmin((_a = context.auth) !== null && _a !== void 0 ? _a : undefined);
    const { publicKey, secretKey } = data;
    if (!secretKey) {
