...
function sendJson(res: functions.Response, status: number, body: unknown) {
  res.set("Content-Type", "application/json; charset=utf-8");
  res.set("Access-Control-Allow-Origin", "*");
  res.status(status).send(JSON.stringify(body));
}

function parseBody(req: functions.https.Request): Record<string, unknown> {
  if (!req.body) return {};
...
function extractCreds(req: functions.https.Request): { key: string; secret: string } | null {
  // 1) Basic Auth
  ...
  // 2) Query params
  const qKey = (req.query.consumer_key as string) || "";
  const qSecret = (req.query.consumer_secret as string) || "";
  if (qKey && qSecret) return { key: qKey, secret: qSecret };
  return null;
}

async function authenticate(req: functions.https.Request): Promise<{ ok: boolean; reason?: string }> {
  const creds = extractCreds(req);
  if (!creds) return { ok: false, reason: "Missing consumer_key / consumer_secret" };
  const snap = await admin.firestore().doc(KEYS_DOC).get();
  if (!snap.exists) return { ok: false, reason: "Integration not configured" };
  const data = snap.data() as {
    consumerKey?: string;
    consumerSecretHash?: string;
    enabled?: boolean;
  };
  if (data.enabled === false) return { ok: false, reason: "Integration disabled" };
...
function productToWoo(id: string, p: FirestoreProduct) {
  const price = Number(p.price ?? 0);
  const regular = Number(p.oldPrice ?? p.price ?? 0);
  ...
  return {
    ...
    menu_order: 0,
    meta_data: [
      { id: 1, key: "supplier_price", value: p.supplierPrice ?? null },
    ],
  };
}

function wooToProduct(body: Record<string, unknown>): FirestoreProduct {
  const out: FirestoreProduct = {};
  ...
  if (body.regular_price != null) out.oldPrice = Number(body.regular_price);
  if (body.sale_price != null && body.sale_price !== "") out.price = Number(body.sale_price);
  else if (body.price != null) out.price = Number(body.price);
  if (body.stock_quantity != null) out.stock = Number(body.stock_quantity);
  if (typeof body.sku === "string") out.sku = body.sku;
  if (typeof body.featured === "boolean") out.featured = body.featured;
  if (Array.isArray(body.images)) {
    out.images = (body.images as Array<{ src?: string } | string>)
      .map((img) => (typeof img === "string" ? img : img.src || ""))
      .filter(Boolean);
  }
  if (Array.isArray(body.categories) && body.categories.length > 0) {
    const c = body.categories[0] as { name?: string; slug?: string };
...
function statusToWoo(s?: string): string {
...
function wooStatusToInternal(s?: string): string | undefined {
...
function orderToWoo(id: string, o: FirestoreOrder) {
...
// ---------- Main handler ----------
export const wcApi = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set(
  ...
  try {
    ...
    if (parts[0] === "products") {
      ...
      if (parts[1] === "categories") {
        if (parts.length === 2 && method === "GET") {
          ...
          const cats = snap.docs.map((d) => {
            const c = d.data() as { name?: string; nameEn?: string };
            return {
              id: d.id,
      ...
      if (parts.length === 1 && method === "GET") {
        ...
        let items = docs.map((d) => productToWoo(d.id, d.data() as FirestoreProduct));
        if (search) {
      ...
      if (parts.length === 1 && method === "POST") {
        ...
        sendJson(res, 201, productToWoo(ref.id, created.data() as FirestoreProduct));
        return;
      }
      ...
      if (parts.length === 2) {
        ...
        if (method === "GET") {
          sendJson(res, 200, productToWoo(id, doc.data() as FirestoreProduct));
          return;
        }
        if (method === "PUT" || method === "PATCH") {
          ...
          sendJson(res, 200, productToWoo(id, fresh.data() as FirestoreProduct));
          return;
        }
        if (method === "DELETE") {
          await ref.delete();
    ...
    if (parts[0] === "orders") {
      ...
      if (parts.length === 1 && method === "GET") {
        ...
        if (status) {
          ...
          if (internal) q = q.where("status", "==", internal);
        }
        if (after) {
        ...
        const items = docs.map((d) => orderToWoo(d.id, d.data() as FirestoreOrder));
        res.set("X-WP-Total", String(items.length));
        res.set("X-WP-TotalPages", "1");
        sendJson(res, 200, items);
        return;
      ...
      if (parts.length === 2) {
        ...
        if (method === "GET") {
          sendJson(res, 200, orderToWoo(id, doc.data() as FirestoreOrder));
          return;
        }
        if (method === "PUT" || method === "PATCH") {
          const body = parseBody(req);
          ...
          if (Array.isArray(body.meta_data)) {
            for (const m of body.meta_data as Array<{ key?: string; value?: unknown }>) {
              if (m?.key === "_tracking_number" && typeof m.value === "string") {
                update.trackingNumber = m.value;
              }
              if (m?.key === "_tracking_url" && typeof m.value === "string") {
                update.trackingUrl = m.value;
              }
              if (m?.key === "_carrier" && typeof m.value === "string") {
          ...
          const fresh = await ref.get();
          sendJson(res, 200, orderToWoo(id, fresh.data() as FirestoreOrder));
          return;
        }
      }
    }
