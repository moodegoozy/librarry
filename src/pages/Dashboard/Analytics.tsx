...
import { subscribeToOrders, subscribeToProducts } from '../../services/firestore';
...
const Analytics: React.FC = () => {
  ...
  const getMonthlyReports = (): MonthlyReport[] => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const productsSold = monthOrders.reduce((sum, order) => 
  ...
  const lastMonthReport = currentMonth > 0 ? monthlyReports[currentMonth - 1] : null;
  ...
  const revenueChange = getPercentChange(currentMonthReport.revenue, lastMonthReport?.revenue ?? null);
  const ordersChange = getPercentChange(currentMonthReport.orders, lastMonthReport?.orders ?? null);
  ...
  const handleExportReport = () => {
    const csvContent = [
      ...
      ...monthlyReports.map(r => [r.monthName, r.revenue, r.orders, r.products])
      ...
      <div className="summary-cards">
        <div className="summary-card">
          ...
          <div className="card-content">
            ...
            <span className="card-value">{formatPrice(currentMonthReport.revenue)}</span>
            <span className={`card-change ${revenueChange >= 0 ? 'positive' : 'negative'}`}>
        ...
        <div className="summary-card">
          ...
          <div className="card-content">
            ...
            <span className={`card-change ${ordersChange >= 0 ? 'positive' : 'negative'}`}>
              {ordersChange >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
          ...
          <table className="reports-table">
            ...
            <tbody>
              {monthlyReports.map((report, index) => (
                <tr key={report.month} className={index === currentMonth ? 'current-month' : ''}>
                  <td>
                    <span className="month-name">{report.monthName}</span>
                    {index === currentMonth && <span className="current-badge">الشهر الحالي</span>}
                  </td>
                  <td className="revenue-cell">{formatPrice(report.revenue)}</td>
                  <td>{report.orders}</td>
                  <td>{report.products}</td>
                  <td>{report.orders > 0 ? formatPrice(report.revenue / report.orders) : '-'}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr>
                ...
                <td className="revenue-cell"><strong>{formatPrice(yearlyRevenue)}</strong></td>
                <td><strong>{yearlyOrders}</strong></td>
                <td><strong>{yearlyProducts}</strong></td>
                <td><strong>{yearlyOrders > 0 ? formatPrice(yearlyRevenue / yearlyOrders) : '-'}</strong></td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <div className="chart-section">
        ...
        <div className="chart-container">
          <div className="chart-bars">
            {monthlyReports.map((report, index) => {
              const maxRevenue = Math.max(...monthlyReports.map(r => r.revenue), 1);
              ...
              return (
                <div key={report.month} className="chart-bar-wrapper">
                  ...
                  >
                    <span className="bar-value">{report.revenue > 0 ? formatPrice(report.revenue) : ''}</span>
                  </div>
                  <span className="bar-label">{report.monthName.slice(0, 3)}</span>
                </div>
              );
            })}
          </div>
